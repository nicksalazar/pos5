...
class ReporteVentasCollection extends ResourceCollection
{
    ...
    public function toArray($request) {
        ...
        return $this->collection->transform(function($row, $key){
            ...
            return [
                ...
                'time_of_issue' => $row->time_of_issue?$row->time_of_issue:null,
                ...
                'items' => $row->items->transform(function($row, $key) {
                    $prod = Item::find($row -> item_id);
                    $cat = Category::find($prod -> category_id);
                    $marca = Brand::find($prod-> brand_id);
                    return [
                        'key' => $key + 1,
                        'id' => $row->id,
