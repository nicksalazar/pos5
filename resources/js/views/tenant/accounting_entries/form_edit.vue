  ...
  <div class="card mb-0 pt-2 pt-md-0">
    <div class="page-header pr-0">
      ...
      <ol class="breadcrumbs">
        <li class="active"><span>Editar Asiento Contable</span></li>
      </ol>
    </div>
    <div class="row flex justify-content-center" v-if="loading_form">
...
import AccountOptions from './partials/options.vue'
 ...
 props: {
  ...
  methods: {
    ...
    async submit() {
      ...
      this.form.total_debe=this.total_debe;
      this.form.total_haber=this.total_haber;
      ...
      this.loading_form=true;
            this.$http.get(`/${this.resource}/record/${this.resourceId}`)
                .then(response => {
                  ...
                  this.form.seat_date=dato.seat_date;
                  this.form.seat=dato.seat;
                  this.form.currency_type_id=dato.currency_type_id;
                  this.form.seat_general=dato.seat_general;
                  this.form.external_id=dato.external_id;
                  this.form.filename=dato.filename;
                  this.form.comment=dato.comment;
                  this.form.types_accounting_entrie_id=dato.types_accounting_entrie_id;
                  this.form.is_client=dato.is_client;
                  if(this.form.is_client==0){
                    this.customer_id=dato.person_id;
                  }else{
                    this.supplier_id=dato.person_id;

                  }
                   this.form.items = dato.detalles.map((item, index) => {
                     item.cuenta = item.account_movement.code+" "+item.account_movement.description;
                      return item;
                    });
                    this.form.total_debe=dato.total_debe;
                    this.form.total_haber=dato.total_haber;
